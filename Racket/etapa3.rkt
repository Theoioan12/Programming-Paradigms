...
(define (get-unstable-couples engagements mpref wpref)
  (let ([u-engagements (map (lambda (x) (cons (cdr x) (car x)))engagements)])
  (filter (lambda (x)(or (better-match-exists? (car x) (cdr x) (get-pref-list wpref (car x)) mpref u-engagements)
...
(define (engage free-men engagements mpref wpref)
  ; prima iteratie
  (let it ((x free-men) (res engagements))
    ; daca lista e nula intorc direct engagements
    (if (null? x) res
        ; primul barbat din lista si lista preferintelor lui
        (letrec ((first (car x)) (wlist (get-pref-list mpref first)))
          ; a doua iteratie
          ; pentru barbatul respectiv si lista de preferinte
         (let it2 ((first first) (wlist wlist))
           (if
            ; daca lista preferintelor e nula ma intorc in it
             (null? wlist) (it (rest x) res)
             ; daca nu
             ; gasesc partenerul si persoana preferata
             (letrec ((wfirst (car wlist)) (z (get-partner res wfirst)))
                (cond
                  ; daca nu e logodita adauga perechea la engagements
                  ((not z) (it (cdr x) (cons (cons wfirst first) res)))
                  ; daca preferinta lui il prefera fac update-engagements
                  ((preferable? (get-pref-list wpref wfirst) first z)
                   (it (cons z (cdr x)) (update-engagements res wfirst first)))
                  ; altfel ma intorc in it2
                  (else (it2 first (cdr wlist)))))))))
   ))
...
(define (get-couple-members pair-list)
  (foldl (lambda (x acc) (append (list (car x)) (list (cdr x)) acc) ) '() pair-list))
