...
;transpun practic "matricea" de preferinte si extrag primul element
; care o sa contina de fapt numele barbatilor
...
;fac foldr folosind cons si practic adaug fiecare
;element din lista
(define (get-women wpref)
  (foldr cons '() (map car wpref)))
...
(define (get-pref-list pref person)
   (cdr (car (filter (λ (x) (member person x)) pref))))
...
; daca y face parte din (member x pref-list) inseamna
; ca e situat dupa x, fiindca member va intoarce elementele care urmeaza
; dupa elementul trimis ca parametru
(define (preferable? pref-list x y)
  (if (member y (member x pref-list)) #t #f))
...
(define (find-first p L)
  (cond
    ;cand lista e nula intorc fals
    ((null? L) #f)
    ;verific daca primul element din lista respecta predicatul
    ;si intorc true daca se verifica
    ((p (car L)) (car L))
    ;altfel aplic recursivitatea pe restul listei
...
(define (get-partner engagements person)
  ; aplic find-first
  (define f (find-first (lambda (x) (equal? person (car x))) engagements))
  ; daca intoarce o lista apelez cdr pentru a obtine perechea
  ;daca nu intorc false
  (and f (cdr f)
        ))
  ;(cond ((null? engagements) #f)
   ;     (else (car (list (find-first (λ (x) (equal? person (car x))) engagements)))))) ;(cdr (find-first (λ (x) (equal? person (car x))) engagements)))
        ;(else (find-first (λ (x) (equal? person (car x))) (cdr engagements)))
    ;))
...
(define (change-first-helper p L val acc)
  (cond
    ; daca list e nula trimit acumulatorul inversat practic
    ((null? L) (reverse acc))
    
    ; daca primul element din lista respecta predicatul
    ((equal? (p (car L)) #t) (reverse (append (reverse(cdr L)) (cons val acc))))
    ; altfel apelez recursiv functia
    ;adaugand la acumulator elementul curent
    (else (change-first-helper p (cdr L) val (cons (car L) acc)))
    ))
...
(define (better-match-exists? p1 p2 p1-list pref2 engagements)
  (cond
    ((null? p1-list) #f)
    ((equal? p2 (car p1-list)) #f)
        ((equal? (and (preferable? p1-list (car p1-list) p2) (preferable? (get-pref-list pref2 (car p1-list)) p1 (get-partner engagements (car p1-list)))) true) #t)
...
; - fiecare cuplu din lista engagements are pe prima poziție
;   o femeie
(define (stable-match-helper engagements mpref wpref acc)
  (cond ((null? engagements) acc)
  ((better-match-exists? (car mpref) (car wpref)))
   ))

(define (stable-match? engagements mpref wpref)
  ;(cond ((null? engagements) #t);((and (null? mpref) (null? wpref)) #t)
  (null? (filter (lambda (x)(or (better-match-exists? (car x) (cdr x) (get-pref-list wpref (car x)) mpref (map (lambda (x) (cons (cdr x) (car x)))engagements))
