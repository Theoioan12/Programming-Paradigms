...
(define (match person engagements pref1 pref2 queue)
  ; am setat parametrii
  ; am pastrat in lista persoanele care nu se afla in coada(sunt in camera)
  ; am obtinut lista de preferinte a primei persoane
  (let loop [(pref-list (filter (λ (x) (not (member x queue))) (get-pref-list pref1 person))) (engagements engagements)]
    (if (null? pref-list) (cons (cons #f person) engagements)
        (let* [(attempt (car pref-list)) (pref-attempt (get-pref-list pref2 attempt))
                                         (partner-attempt (get-partner engagements attempt))]
          ; daca cineva e cuplat cu fals, inseamna ca poate fi cuplat direct
          (cond
            [(equal? partner-attempt #f) (update-engagements engagements attempt person)]
            ; verific daca nu e casatorit cu fals
            ; e preferata persoana curenta fata de partenerul curent
            ; daca e, ii dau match
            [ (preferable? pref-attempt person partner-attempt) (match partner-attempt (update-engagements engagements attempt person) pref1 pref2 queue)]
            ; daca nu, continui iteratia
            [else (loop (cdr pref-list) engagements)]
...
(define (path-to-stability engagements mpref wpref queue)
  ; coada e nula si intorc direct engagements
  (if (null? queue) engagements
      ; daca persoana din coada face parte din lista de preferinte feminine
    (if (member (car queue) (get-women wpref))
        ; intorc perechile invers (pt femei) si apoi reverse la loc dupa ce am generat matchul
       (path-to-stability (map (lambda (x) (cons (cdr x) (car x))) (match (car queue) (map (lambda (x) (cons (cdr x) (car x)))engagements) wpref mpref queue)) mpref wpref (cdr queue))
       ; altfel aplic functia match pe persoanele din afara camerei
       (path-to-stability (match (car queue) engagements mpref wpref queue) mpref wpref (cdr queue))
    )))
...
(define (update-stable-match engagements mpref wpref)
  ; aplic doar path-to-stability
  (let* [(unstable (get-unstable-couples engagements mpref wpref)) (u-engagements (filter (λ (x) (not (member x unstable))) engagements)) (couple-members (get-couple-members unstable)) 
                                                                   ]
    (path-to-stability u-engagements mpref wpref couple-members)
...
; punctajul pe acest exercițiu se anulează în totalitate.
 (define (build-stable-matches-helper pref-stream)
    (let* [(mfirst (car pref-stream))
           (wfirst (cdr pref-stream))
           (algo (gale-shapley mfirst wfirst))
           (applied (update-stable-match algo mfirst wfirst))]
      applied))

(define (build-stable-matches-stream pref-stream)
  (stream-map build-stable-matches-helper pref-stream))
